...
module GDK
  class Config < ConfigSettings
    ...
    settings :postgresql do
      ...
      string(:__host_for_daemon) { GDK::PostgreSQL.new.use_tcp? ? config.postgresql.host : '' }
      ...
      settings :geo do
        ...
        string(:__host_for_daemon) { GDK::GeoPostgreSQL.new.use_tcp? ? config.postgresql.geo.host : '' }
      end
    end
