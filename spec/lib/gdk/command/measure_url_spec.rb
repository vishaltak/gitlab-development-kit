    ...
    context 'when GDK is running' do
      ...
      it 'runs sitespeed via Docker for the given URL', :hide_stdout do
        freeze_time do
          ...
          expect(Shellout).to receive(:new).with(%(docker run #{network_host} --cap-add=NET_ADMIN --shm-size 2g --rm -v "$(pwd):/sitespeed.io" sitespeedio/sitespeed.io:1.2.3 -b chrome -n 4 -c cable --cookie perf_bar_enabled=false --cpu --outputFolder #{report_file_path} http://host.docker.internal:3000/explore)).and_return(shellout_docker_run_double)
