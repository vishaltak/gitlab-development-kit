...
RSpec.describe GDK::Config do
  ...
  describe '#postgresql' do
    ...
    describe '#active_version' do
      it { expect(default_config.postgresql.active_version).to eq('12.10') }
    ...
    describe '#image' do
      context 'when no image is specified' do
        it 'returns the default image' do
          expect(config.registry.image).to eq('registry.gitlab.com/gitlab-org/build/cng/gitlab-container-registry:v3.49.0-gitlab')
        end
      end
    end
