...
RSpec.describe GDK::Dependencies do
  ...
  describe '.asdf_available?' do
    ...
    context 'when asdf binary is available in PATH' do
      before do
      ...
      it 'returns false when neither ASDF_DATA_DIR nor ASDF_DIR are present' do
        stub_env('ASDF_DIR', nil)
        stub_env('ASDF_DATA_DIR', nil)
    ...
    it 'returns false when asdf binary is not available in PATH' do
      expect(subject).to be_falsey
    end
  end
